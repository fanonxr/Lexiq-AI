...
class RabbitMQSettings(BaseSettings):
    ...
    queue_name: str = Field(
        ...
        description="Queue name for ingestion jobs",
        alias="QUEUE_NAME",
    )
    ...
    routing_key: str = Field(
        default="document.ingestion",
        description="Routing key for messages",
        alias="ROUTING_KEY",
    )
    prefetch_count: int = Field(
        default=10,
        description="Number of unacknowledged messages per worker",
        alias="PREFETCH_COUNT",
    )
    queue_durable: bool = Field(
        default=True, description="Make queue durable", alias="QUEUE_DURABLE"
    )
    message_ttl: Optional[int] = Field(
        default=None,
        description="Message TTL in milliseconds (None = no TTL)",
        alias="MESSAGE_TTL",
    )
...
class EmbeddingSettings(BaseSettings):
    ...
    provider: EmbeddingProvider = Field(
        ...
        description="Embedding provider: openai or azure",
    ...
    openai_api_key: Optional[str] = Field(
        default=None,
        description="OpenAI API key (direct) for embeddings",
        alias="OPENAI_API_KEY",
    )
    # Optional alternate env var name some users use
    open_ai_api_key: Optional[str] = Field(
        default=None,
        description="Alternate OpenAI API key env var name (OPEN_AI_API_KEY)",
        alias="OPEN_AI_API_KEY",
    )
    openai_base_url: Optional[str] = Field(
        default=None,
        description="Optional OpenAI base URL (advanced)",
        alias="OPENAI_BASE_URL",
    )
    open_ai_base_url: Optional[str] = Field(
        default=None,
        description="Alternate OpenAI base URL env var name (OPEN_AI_BASE_URL)",
        alias="OPEN_AI_BASE_URL",
    )

    # Azure OpenAI embeddings (optional)
    azure_openai_endpoint: Optional[str] = Field(
        default=None, description="Azure OpenAI endpoint URL", alias="AZURE_OPENAI_ENDPOINT"
    )
    azure_openai_api_key: Optional[str] = Field(
        default=None, description="Azure OpenAI API key", alias="AZURE_OPENAI_API_KEY"
    )
    ...
    embedding_model: str = Field(
        ...
        description="Embedding model name (OpenAI direct)",
        alias="EMBEDDING_MODEL",
    )
    embedding_deployment_name: Optional[str] = Field(
        default=None,
        description="Embedding deployment name (Azure OpenAI)",
        alias="EMBEDDING_DEPLOYMENT_NAME",
    )
    embedding_dimension: Optional[int] = Field(
        default=None,
        description="Optional embedding dimension (used for validation / Qdrant collection sizing)",
    ...
    def is_configured(self) -> bool:
        ...
        if self.provider == EmbeddingProvider.AZURE:
...
class QdrantSettings(BaseSettings):
    ...
    collection_prefix: str = Field(
        default="firm_",
        description="Prefix for collection names (firm_{firm_id})",
        alias="COLLECTION_PREFIX",
    )
...
class ChunkingSettings(BaseSettings):
    ...
    chunk_size: int = Field(
        default=1000, description="Chunk size in tokens", alias="CHUNK_SIZE"
    )
    chunk_overlap: int = Field(
        default=200, description="Overlap between chunks in tokens", alias="CHUNK_OVERLAP"
    )
    chunking_method: str = Field(
        default="sentence",
        description="Chunking method: sentence, paragraph, or fixed",
        alias="CHUNKING_METHOD",
    )
...
class RetrySettings(BaseSettings):
    ...
    max_retries: int = Field(
        default=3, description="Maximum number of retries", alias="MAX_RETRIES"
    )
    ...
    max_delay: int = Field(
        default=300, description="Maximum delay between retries in seconds", alias="RETRY_MAX_DELAY"
    )
...
class APICoreSettings(BaseSettings):
    ...
    timeout: int = Field(
        default=30, description="Request timeout in seconds", alias="TIMEOUT"
    )
    api_key: Optional[str] = Field(
        default=None,
        description="Internal API key for calling API Core internal endpoints (sent as X-Internal-API-Key)",
        alias="API_KEY",
    )
...
class ServerSettings(BaseSettings):
    ...
    port: int = Field(default=8003, description="HTTP server port", alias="PORT")
    reload: bool = Field(
        default=False, description="Enable auto-reload (development only)", alias="RELOAD"
    )
...
class Settings(BaseSettings):
    ...
    # Application settings
    app_name: str = Field(
        default="document-ingestion", description="Application name", alias="APP_NAME"
    )
    ...
    allowed_file_types_str: Optional[str] = Field(
        ...
        description="Allowed file types for processing (comma-separated)",
